...
class DataQualityOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if num_records < 1:
            ...
            self.log.info(f"Data quality on table songplays check passed with {num_records} records")
        ...
        if self.data_quality_check: 
            ...
            try:
                for check in self.data_quality_check:
                    ...
                    expected_result = check.get('expected_value', 0)  # get check['expected_result'] if exists, else assume 0
                    
                    if check_result == expected_result :                    
                        self.log.info(f"additional data quality ckeck {check_index} passed")
                    else:
                        self.log.info(f"additional data quality ckeck failed, returned:{check_result[0][0]},expected:{expected_result}")
                    check_index += 1
            except NameError:
